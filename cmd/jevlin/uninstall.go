...
const uninstallUsage = `usage: jevlin uninstall [-home dir] [-binary] [-purge-state] [-dry-run] [-yes]
...
// binary they run.
const uninstallProbeCommand = "\x00jevlin-uninstall-probe"
...
func (r *uninstallRun) fail(format string, args ...any) {
	r.failures++
	fmt.Fprintf(r.d.stderr, "jevlin uninstall: "+format+"\n", args...)
}
...
func uninstallMain(d uninstallDeps, args []string) int {
	...
	if fs.NArg() > 0 {
		fmt.Fprintf(d.stderr, "jevlin uninstall: unexpected argument %q\n%s", fs.Arg(0), uninstallUsage)
		return exitUsage
	}
...
func (r *uninstallRun) run(homeFlag string) int {
	...
	if err != nil {
		fmt.Fprintln(d.stderr, "jevlin uninstall:", err)
		return exitUsage
	}
	r.home = home
	...
	if r.purge && !r.dry && !d.interactive {
		fmt.Fprintln(d.stderr, "jevlin uninstall: -purge-state needs a terminal: it asks you to type a confirmation, and nothing else can answer it (-yes cannot). Nothing was changed.")
		return exitUsage
	}
	if r.binary {
		if exeErr != nil {
			fmt.Fprintln(d.stderr, "jevlin uninstall: cannot determine my own path:", exeErr)
			return exitTransport
		}
		if err := r.checkBinaryOwnership(); err != nil {
			fmt.Fprintf(d.stderr, "jevlin uninstall -binary: %v\nNothing was changed.\n", err)
			return exitUsage
		}
	}
	if r.purge {
		if err := checkPurgeTarget(home, d.userHome); err != nil {
			fmt.Fprintf(d.stderr, "jevlin uninstall -purge-state: %v\nNothing was changed.\n", err)
			return exitUsage
		}
	}
	if !r.dry && !r.purge && !d.interactive && !r.yes {
		fmt.Fprintln(d.stderr, "jevlin uninstall: not an interactive shell; pass -yes to remove what is listed by -dry-run. Nothing was changed.")
		return exitUsage
	}
...
func (r *uninstallRun) checkBinaryOwnership() error {
	if kind := classifyLaunch(r.exe, r.d.getenv("JEVLIN_LAUNCH")); kind != launchNative {
...
func (r *uninstallRun) printExclusionError(err error) {
	...
	default:
		r.fail("cannot coordinate with other Jevlin commands for %s: %v. %s", r.home, err, noParticipantChange)
	}
}
...
func (r *uninstallRun) plan() {
	...
	if r.d.windows {
		...
		for _, e := range edits {
			r.printf("  remove the jevlin block from %s\n", tilde(r.d.userHome, e.display))
		}
		if len(edits) == 0 && len(notes) == 0 {
			r.printf("  no jevlin block for this installation\n")
		}
		for _, n := range notes {
			r.printf("  %s\n", n)
		}
...
// profileEdit is one profile losing its jevlin block.
...
func (r *uninstallRun) profilePlan() (edits []profileEdit, notes []string) {
	...
	for _, name := range []string{".zshrc", ".bashrc"} {
		...
		if errors.Is(err, errProfileMalformed) {
			notes = append(notes, fmt.Sprintf("not touching %s: %v. Remove the jevlin lines between %q and %q by hand",
...
func (r *uninstallRun) applyEnvironment() {
	...
	for _, e := range edits {
		if err := fsx.WriteFileAtomic(filepath.Dir(e.target), filepath.Base(e.target), e.next, e.mode); err != nil {
			r.fail("could not remove the jevlin block from %s: %v", e.display, err)
			continue
		}
		r.printf("removed the jevlin block from %s\n", tilde(r.d.userHome, e.display))
	}
	for _, n := range notes {
		r.printf("%s\n", n)
	}
...
func (r *uninstallRun) applyBinary() {
	...
	for _, p := range r.binarySet() {
		...
		case p == owned && r.d.windows:
			...
			r.printf("moved %s aside to %s: Windows cannot delete a running binary; delete that file once no Jevlin or agent process is running it\n", owned, aside)
			continue
		}
		if err := os.RemoveAll(p); err != nil {
	...
	if !r.purge {
		...
		if !r.binary {
			...
			r.printf("\nTo keep using this installation, run:\n"+
				...
				"Do not run `jevlin connect` on its own to come back: with nothing naming this\n"+
...
func (r *uninstallRun) sayLeftoverLocks() {
	...
	r.printf("\nLeft, and safe to delete — the lock files Jevlin commands coordinate through.\n" +
