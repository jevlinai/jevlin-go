...
func resolvePendingTxLocked(ctx context.Context, c *rpcClient, dir string, stdout, stderr io.Writer) pendingOutcome {
	p, err := loadPendingTx(dir)
	if err != nil {
		fmt.Fprintln(stderr, "jevlin: pending transaction record:", err)
	...
	if err != nil {
		fmt.Fprintf(stderr, "jevlin: a previous send (%s) is still unresolved and could not be checked: %v\n", p.Hash, err)
		return pendingCheckFailed
	}
	if confirmed {
		...
		if err := removePendingTx(dir); err != nil {
			fmt.Fprintln(stderr, "jevlin: could not clear the resolved pending transaction:", err)
			return pendingCheckFailed
		}
	...
	txRaw, err := p.txRaw()
	if err != nil {
		fmt.Fprintln(stderr, "jevlin: pending transaction record is corrupt:", err)
		return pendingCheckFailed
	}
