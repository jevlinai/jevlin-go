...
$assemblyName = New-Object System.Reflection.AssemblyName 'JevlinSecondPrincipalProbe'
$assembly = [AppDomain]::CurrentDomain.DefineDynamicAssembly($assemblyName, [System.Reflection.Emit.AssemblyBuilderAccess]::Run)
$module = $assembly.DefineDynamicModule('JevlinSecondPrincipalProbe')
$typeBuilder = $module.DefineType('JevlinSecondPrincipalProbe.Native', [System.Reflection.TypeAttributes]'Public, Class')
...
$paths = $env:JEVLIN_PROBE_PATHS.Split([char]'|')
$kinds = $env:JEVLIN_PROBE_KINDS.Split([char]'|')
...
func probeAsSecondPrincipal(t *testing.T, targets ...principalProbe) []uint32 {
	...
	cmd.Env = append(os.Environ(), "JEVLIN_PROBE_PATHS="+strings.Join(paths, "|"), "JEVLIN_PROBE_KINDS="+strings.Join(kinds, "|"))
...
func TestDoctorReportsAnotherReaderOfTheWalletUntilSetupRepairsIt(t *testing.T) {
	...
	if check["fix"] != "jevlin setup" {
		t.Errorf("fix = %v, want jevlin setup", check["fix"])
	}
