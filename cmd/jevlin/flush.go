...
//
//	jevlin flush [-config file] [-force] [-timeout 90s]
//	jevlin flush -detach ...     start it and return immediately
//
...
import (
	...
	"github.com/jevlinai/jevlin-go/pkg/mining/promote"
...
func saveFlushStamp(store *auth.Store, path string, st flushStamp, stderr io.Writer) error {
	if err := writeFlushStamp(path, st); err != nil {
		fmt.Fprintln(stderr, "jevlin flush: stamp:", err)
		if store != nil {
...
func cmdFlush(args []string, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if *detach {
		if err := startFlush(*cfgPath); err != nil {
			...
			fmt.Fprintln(stderr, "jevlin flush: could not start:", err)
	...
	if err != nil {
		fmt.Fprintln(stderr, "jevlin flush:", err)
		return exitTransport
	}
	...
	if err != nil {
		...
		fmt.Fprintf(stderr, "jevlin flush: %v (%s); nothing was flushed, run it again shortly\n", err, gatePath)
		return exitTransport
	}
	defer gate.release()
	...
	if err != nil {
		fmt.Fprintf(stderr, "jevlin flush: config (%s): %v\n", orDefaults(src), err)
		return exitTransport
	}
	if !cfg.Miner.Enabled {
		fmt.Fprintf(stderr, "jevlin flush: [miner] is not enabled in %s; the daemon flushes for itself\n", orDefaults(src))
		return exitUsage
	}
...
func runFlushAdmitted(ctx context.Context, cfg *config.Config, cfgPath string, force bool, stdout, stderr io.Writer, endAdmission func()) (flushReport, int) {
	...
	if err := os.MkdirAll(minerRoot(mn), 0o700); err != nil { // #nosec G703 -- the configured intake directory's parent
		...
		fmt.Fprintln(stderr, "jevlin flush: state dir:", err)
	...
	if err != nil {
		markFlushStateHealth(m.StateDir, flushLockHealthPrefix+err.Error())
		fmt.Fprintf(stderr, "jevlin flush: lock %s: %v\n", flushLockPath(mn), err)
		return rep, exitTransport
	}
	if !held {
	...
	if store == nil && decision.State == auth.MiningDegraded {
		fmt.Fprintf(stderr, "jevlin flush: mining state cannot be safely trusted%s\n", miningDecisionDetail(decision))
		return rep, exitTransport
	}
	...
	if !enrolled() {
		...
		fmt.Fprintln(stderr, "jevlin flush: this installation is not enrolled; run: jevlin enroll")
	...
	if err != nil {
		...
		fmt.Fprintln(stderr, "jevlin flush: spool:", err)
		return rep, exitTransport
	}
	...
	promotionErr := err
	if err != nil {
		fmt.Fprintln(stderr, "jevlin flush: intake:", err)
	}
