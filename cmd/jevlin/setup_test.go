...
// jevlin setup, driven in-process against a sandbox: a fake user home,
...
func newSetupSandbox(t *testing.T) *setupSandbox {
	...
	s := &setupSandbox{
		...
		exe:      filepath.Join(root, "bin", "jevlin"),
...
func TestSetupInteractiveYesToEverything(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		...
		t.Errorf("profile holds %d jevlin blocks, want 1", n)
	}
	if !strings.Contains(out, "     jevlin search -format model") {
...
func TestSetupRefusesNpmCopiesThatWillBeDiscarded(t *testing.T) {
	cases := []struct {
		...
		{"/usr/local/bin/jevlin", "", true},
		{"/usr/local/lib/node_modules/jevlin/bin/jevlin", "npm:global", true},
		{"/home/u/proj/node_modules/jevlin/bin/jevlin", "npm:local", false},
		{"/home/u/proj/node_modules/jevlin/bin/jevlin", "npm:unknown", false},
		{"/home/u/.npm/_npx/9f/node_modules/jevlin/bin/jevlin", "npm:local", false},
		{"/home/u/.npm/_npx/9f/node_modules/jevlin/bin/jevlin", "npm:global", false},
		{`C:\Users\u\AppData\Local\npm-cache\_npx\9f\node_modules\jevlin\bin\jevlin.exe`, "", false},
		{"/home/u/.npm/_cacache/tmp/jevlin", "", false},
		{"/opt/jevlin", "something-else", false},
		{"/usr/local/lib/node_modules/jevlin/bin/jevlin", "", false},
	...
	s := newSetupSandbox(t)
	s.env["JEVLIN_LAUNCH"] = "npm:local"
	before := snapshotTree(t, s.root)
	code, _, errOut := s.run(nil, false)
	if code != exitUsage || !strings.Contains(errOut, "npm install -g jevlin") {
		t.Fatalf("a project-local npm copy was not refused: exit %d\n%s", code, errOut)
	}
	delete(s.env, "JEVLIN_LAUNCH")
	s.exe = filepath.Join(s.root, "node_modules", "jevlin", "bin", "jevlin")
	code, _, errOut = s.run(nil, false)
	if code != exitUsage || !strings.Contains(errOut, "run the jevlin command npm installed") {
...
const cases = [
  ["/usr/local/lib/node_modules/jevlin", "/usr/local/bin/node", {}, "linux", has([]), "global"],
  ["/home/u/.nvm/versions/node/v22/lib/node_modules/jevlin", "/home/u/.nvm/versions/node/v22/bin/node", {}, "linux", has([]), "global"],
  ["/home/u/.npm-global/lib/node_modules/jevlin", "/usr/bin/node", {npm_config_prefix: "/home/u/.npm-global"}, "linux", has([]), "global"],
  ["/home/u/.npm-global/lib/node_modules/jevlin", "/usr/bin/node", {}, "linux", has(["/home/u/.npm-global/bin/jevlin"]), "global"],
  ["/home/u/proj/node_modules/jevlin", "/usr/local/bin/node", {}, "linux", has([]), "local"],
  ["/home/u/lib/node_modules/jevlin", "/usr/local/bin/node", {}, "linux", has([]), "local"],
  ["/home/u/.npm/_npx/9f/node_modules/jevlin", "/usr/local/bin/node", {}, "linux", has([]), "local"],
  ["/opt/unpacked/jevlin", "/usr/local/bin/node", {}, "linux", has([]), "unknown"],
  ["C:\\Users\\u\\AppData\\Roaming\\npm\\node_modules\\jevlin", "C:\\Program Files\\nodejs\\node.exe", {}, "win32", has(["C:\\Users\\u\\AppData\\Roaming\\npm\\jevlin.cmd"]), "global"],
...
`
	launcher, err := filepath.Abs(filepath.Join("..", "..", "npm", "bin", "jevlin.js"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetupHelpPrintsUsageAndExitsZero(t *testing.T) {
	for _, flag := range []string{"-h", "--help", "-help"} {
		...
		if !strings.Contains(out.String(), "usage: jevlin setup") || !strings.Contains(out.String(), allTargetIDs()) {
