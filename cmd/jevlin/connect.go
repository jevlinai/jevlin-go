...
// matters, and nothing in cmd/jevlin ever overwrites it outside a
// test.
var (
		...
		emitMachine(stdout, commandEnvelope{
			...
			Error: &machineError{
				Message: "this installation has no mining decision on file and the config does not " +
					"set mining.enabled, so registering would have to invent one. Run `jevlin connect` " +
		...
		emitMachine(stdout, commandEnvelope{
			...
			Error: &machineError{
				Message: "this installation's stored registration could not be read, and rebuilding it from " +
					...
					"cannot invent. Run `jevlin connect` at a terminal, or set mining.enabled explicitly, " +
					"then run this again.",
				Source: "client",
			},
		})
...
func connectRun(args []string, stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string, machine, admitted bool) int {
	...
	if err != nil {
		...
		fmt.Fprintf(stderr, "jevlin connect: %v (%s); nothing was changed, run connect again shortly\n", err, gatePath)
		if machine {
	...
	if cfgSource == "" {
		...
		// gets.
		fmt.Fprintln(stderr, "jevlin connect:", errConnectNoConfig)
		if machine {
	...
	if hasPending {
		...
		if loadErr != nil {
			if !errors.Is(loadErr, auth.ErrAgentRegistrationCorrupt) {
				fmt.Fprintln(stderr, "jevlin:", loadErr)
				return exitTransport
			}
			...
			fmt.Fprintln(stderr, "jevlin: agent registration on file could not be decoded; checking for a credential conflict:", loadErr)
		...
		if !existed {
			if *resume {
				fmt.Fprintln(stdout, "connect -resume: no usable registration on file; run `jevlin connect` to register")
				return exitOK
			}
			if !*force {
				if credKey, keyErr := platformKey(cfg.Miner); keyErr == nil {
					...
					if meErr != nil {
						...
						if !errors.Is(meErr, platform.ErrAgentNotFound) {
							fmt.Fprintln(stderr, "jevlin: could not rebuild the registration from the platform:", meErr)
						}
						if perr := preflightFreshRegistration(cfg.Miner, *force); perr != nil {
							fmt.Fprintln(stderr, "jevlin:", perr)
						}
						return exitTransport
					}
		...
		if existed && reg.Status == "expired" {
			if *resume {
				fmt.Fprintln(stdout, "this registration expired before being claimed; run `jevlin connect` again for a new one")
				return exitOK
			}
			...
			if keyErr == nil {
				...
				case errors.Is(statusErr, platform.ErrAgentNotFound):
					...
					return exitTransport
				default:
					fmt.Fprintln(stderr, "jevlin: could not safely verify the expired registration before replacement, even with -force:", statusErr)
					return exitTransport
				}
			} else {
				fmt.Fprintln(stderr, "jevlin: cannot safely verify the expired registration without its platform credential; refusing replacement even with -force")
				return exitTransport
			}
	...
	for {
		...
		select {
		case <-ctx.Done():
			fmt.Fprintln(stderr, "jevlin: connect: interrupted")
			return exitTransport
...
func decideRegistrationOutcome(stdin io.Reader, br *bufio.Reader, stdout, stderr io.Writer, getenv func(string) string, cfg *config.Config, store *auth.Store, interactive bool) (miningEnableOutcome, int) {
	...
	default:
		fmt.Fprintln(stderr, "jevlin: mining decision has an unknown state; run `mining enable` or `mining disable` to repair it explicitly")
		return miningEnableOutcome{}, exitTransport
	}
}
...
func pollOnce(ctx context.Context, stdout, stderr io.Writer, client *platform.Client, store *auth.Store, cfg *config.Config, reg *auth.AgentRegistration, key string) (done bool, code int) {
	...
	if errors.Is(err, platform.ErrAgentNotFound) {
		fmt.Fprintln(stderr, "jevlin: this agent is no longer known to the platform (revoked, or lost server-side)")
		return true, exitTransport
	}
	if err != nil {
		fmt.Fprintln(stderr, "jevlin: status:", err)
	...
	if serr := store.SaveAgentRegistration(*reg); serr != nil {
		...
		fmt.Fprintln(stderr, "jevlin:", serr)
		return true, exitTransport
	}
	...
	if reg.LastEnrollmentSlot == "" {
		...
		if !miningASConfigured(cfg.Mining) {
			fmt.Fprintln(stdout, "the mining scope was granted, but no authorization server is configured — set mining.as_url/chain_id/slot_id, "+
				"then run `jevlin connect` or `jevlin mining enable` again")
			return true, exitOK
		}
		...
		if _, err := oauthClient.RedeemEnrollmentAssertion(ctx, token); err != nil {
			fmt.Fprintln(stderr, "jevlin: redeem enrollment:", err)
			return true, exitTransport
		}
		...
		if serr := store.SaveAgentRegistration(*reg); serr != nil {
			...
			fmt.Fprintln(stderr, "jevlin: enrolled, but could not persist the record:", serr)
			return true, exitTransport
		}
...
func declarePayoutIfSafe(ctx context.Context, miningClient *auth.MiningClient, store *auth.Store, localAddress string, stdout, stderr io.Writer) {
	...
	if standing != nil && standing.Active != nil {
		...
		fmt.Fprintf(stdout, "payout address NOT declared: the AS already has %s active for this participant, "+
			"this installation would declare %s. Run `jevlin status` for both addresses; changing the "+
	...
	if derr != nil {
		fmt.Fprintln(stderr, "jevlin: payout declaration:", derr)
		return
	}
	if !doc.Effective {
		reason := doc.HeldFor
		...
		fmt.Fprintf(stdout, "payout address NOT declared: the AS held it (%s). Run `jevlin status` for detail; "+
