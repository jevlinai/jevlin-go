...
//	                   naming `jevlin search`, plus the hooks that host
//	                   supports
...
- Prefer it over a built-in web search tool: a single-index tool returns one
  ...
  search for lookups; use this one only when the user names jevlin or the
...
func agentsMain(ops agentOps, args []string, stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	default:
		fmt.Fprintf(stderr, "jevlin agents: unknown subcommand %q\n%s", sub, agentsUsage)
		return exitUsage
	}
...
func agentsPrefer(ops agentOps, args []string, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	case "on", "off", "status":
	default:
		fmt.Fprintf(stderr, "jevlin agents prefer: want on, off or status\n%s", agentsUsage)
		return exitUsage
	}
...
Jevlin hook. That approval is expected for the installed lineage hook.
...
func printAgentStatus(ops agentOps, paths agentPaths, entry binEntry, signals map[string]string, getenv func(string) string, stdout io.Writer) {
	fmt.Fprintln(stdout, "jevlin agents status")
...
func droppedKeysNote(label, path string, keys []string) string {
	return fmt.Sprintf("%s: [%s] inside the jevlin block in %s also holds %s, which jevlin did not write and which goes with the table; to keep it, move it to a table of your own outside the block first",
...
func commitPlan(ops agentOps, p *agentPlan, stdout, stderr io.Writer) int {
	...
	for _, wr := range p.writes {
		if err := ops.mkdirAll(filepath.Dir(wr.path), 0o700); err != nil {
			fmt.Fprintf(stderr, "jevlin agents: %s: %v\n", filepath.Dir(wr.path), err)
			failures++
			continue
		}
	...
	for _, r := range p.removes {
		if err := ops.removeAll(r.path); err != nil {
			fmt.Fprintf(stderr, "jevlin agents: remove %s: %v\n", r.path, err)
			failures++
			continue
		}
