...
func doctorWalletCheck(f walletAccessFacts) doctorCheck {
	...
	if len(f.Readers) > 0 || len(f.Unsecurable) > 0 {
		...
		if len(f.Readers) > 0 {
			c.Detail += "; another program or a machine policy can add such an entry again, and jevlin setup re-applies the protection whenever it does"
		}
		if f.Err != nil {
		...
		if len(f.Unsecurable) > 0 {
			c.Fix = "move " + strings.Join(f.Unsecurable, ", ") + " out of the wallet directory, then run: jevlin setup"
		}
		return c
	}
	if f.Err != nil {
