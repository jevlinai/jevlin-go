...
// part of it another flush can observe.
...
const flushHolderEnv = "JEVLIN_TEST_FLUSH_HOLDER_CONFIG"
...
// mode. The read-write holder stands in for a flush outside a sandbox;
...
func TestPurgeRemovesTheFlushStampAndTheLock(t *testing.T) {
	s := installed(t)
	paths := []string{
		filepath.Join(s.home, "state", "flush.json"),
		filepath.Join(s.home, "flush.lock"),
	}
	writeFileT(t, paths[0], `{"v":1}`)
	if !lexists(paths[1]) {
...
func TestConfiguredStatePathsNameTheFlushStampAndTheLock(t *testing.T) {
	root := t.TempDir()
	cfg := &config.Config{
	...
	for _, want := range []string{flushStampPath(cfg.Mining), flushLockPath(cfg.Miner)} {
		if !named[want] {
...
// blockStamp makes the stamp unwritable by putting a directory where it
...
func TestDoctorReadsTheStampAndReportsAnUnreadableOne(t *testing.T) {
	root := t.TempDir()
	...
	if f := gather(); f.StampPresent || f.StampErr != nil {
		t.Fatalf("no stamp: present %v err %v, want absent", f.StampPresent, f.StampErr)
	}
	at := time.Now().Add(-time.Hour).UTC().Truncate(time.Second)
	writeStampT(t, flushStampPath(m), flushStamp{LastFlush: at})
	if f := gather(); !f.StampPresent || f.StampErr != nil || !f.Stamp.LastFlush.Equal(at) {
		t.Fatalf("stamp: present %v err %v at %s, want %s", f.StampPresent, f.StampErr, f.Stamp.LastFlush, at)
	}
	if err := os.WriteFile(flushStampPath(m), []byte("{"), 0o600); err != nil {
		t.Fatal(err)
	}
	if f := gather(); f.StampErr == nil {
		t.Fatalf("an unreadable stamp was not reported as unreadable: present %v at %s", f.StampPresent, f.Stamp.LastFlush)
	}
}
