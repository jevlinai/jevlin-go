...
// executable: that file is named jevlin.test, and isSearchCommand — the
// gate the hook applies before it looks at anything — matches on the binary
// being named jevlin. The recognizer's identity check only needs the
...
func newHookFixture(t *testing.T, sh shellKind, cfgName string) recognizerFixture {
	...
	bin := filepath.Join(binDir, "jevlin")
