...
// defaultJevlinHome is the installation location this process assumes
// when nothing more specific names one: JEVLIN_HOME, else ~/.jevlin.
...
// installation is.
func defaultJevlinHome(getenv func(string) string) string {
	if home := getenv("JEVLIN_HOME"); home != "" {
		return home
	}
	...
	return filepath.Join(userHome, ".jevlin")
}
