		...
		for _, fixture := range []string{".input.yaml", ".roundtrip.yaml"} {
			...
			for otherName, other := range others {
				t.Run(name+fixture+"/"+otherName, func(t *testing.T) {
					...
					for _, want := range []string{"left the jevlin pre_tool_call hook block", "another installation", owner.cfg} {
	...
	for _, fixture := range []string{"posix.input.yaml", "posix.roundtrip.yaml"} {
		...
		for name, tc := range map[string]struct {
			...
			t.Run(fixture+"/"+name, func(t *testing.T) {
				...
				if notes := strings.Join(uninstall.notes, "\n"); !strings.Contains(notes, where) || !strings.Contains(notes, "is not a line jevlin writes") {
				...
				case tc.comment:
				case !strings.Contains(notes, where) || !strings.Contains(notes, "left the jevlin pre_tool_call hook block"):
...
func TestTwoMarkedBlocksAreNeitherReadNorRemoved(t *testing.T) {
	...
	other := string(hermesHookBlock(strings.Join(hermesHookLines("/somewhere/else/bin/jevlin hook hermes pre_tool_call"), "\n")+"\n", false))
