...
func cmdMining(args []string, stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "jevlin: mining needs a subcommand: enable, disable")
		return exitUsage
	}
	switch args[0] {
	case "enable":
...
func cmdMiningEnable(args []string, stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if !ok {
		fmt.Fprintln(stderr, "jevlin: no agent registration found; run `jevlin connect` first")
		return exitTransport
	}
	...
	if st, serr := client.Status(ctx, reg.AgentID, key); serr == nil {
		...
		fmt.Fprintln(stderr, "jevlin: could not reach the platform to check status; proceeding on what was last known:", serr)
	}
	...
	if !hasScope(reg.Scopes, "mining") {
		...
		default:
			...
			fmt.Fprintln(stdout, "\nOnce granted, this resolves automatically the next time `search` runs, or run `jevlin connect` to check now.")
		}
		return exitOK
	}
...
func cmdMiningDisable(args []string, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if err != nil {
		...
		fmt.Fprintln(stderr, "jevlin: agent registration on file could not be read (treating as absent):", err)
		regOK = false
	}
...
func askMiningQuestion(stdin io.Reader, br *bufio.Reader, stdout, stderr io.Writer, getenv func(string) string, cfg *config.Config, store *auth.Store, interactive, participantHasOtherAgent bool) (miningEnableOutcome, int) {
	...
	if !cfg.MiningEnabledExplicit {
		...
		if err != nil {
			fmt.Fprintf(stderr, "\njevlin: %s; no mining decision was recorded and nothing was registered\n", promptAbortedReason)
...
func finishMiningEnabled(stdin io.Reader, br *bufio.Reader, stdout, stderr io.Writer, getenv func(string) string, cfg *config.Config, store *auth.Store, interactive bool) (miningEnableOutcome, int) {
	...
	if address == "" {
		...
		if _, err := os.Lstat(filepath.Join(dir, walletKeyFile)); err != nil && !errors.Is(err, fs.ErrNotExist) {
			...
			if cerr != nil {
				fmt.Fprintln(stderr, "jevlin:", cerr)
				return miningEnableOutcome{}, exitTransport
			}
