...
func doctorEnrolledCheck(f doctorFacts) doctorCheck {
	...
	case !f.HasRefresh:
		...
		c.Fix = doctorEnrollFix(f, "claim the URL `jevlin connect` printed, then run `jevlin connect` "+
	...
	case f.AuthIncomplete:
		...
		c.Fix = doctorEnrollFix(f, "run `jevlin connect` again to complete local authorization")
	case f.StatusErr != nil && f.DocErr == nil && f.EpochKnown:
		...
		c.Fix = doctorEnrollFix(f, "if this does not clear on its own: jevlin mining disable, then "+
			"jevlin mining enable (or just `connect` again) to get a fresh authorization")
	case f.DocErr != nil:
		c.Verdict = verdictOK
...
func doctorEnrollFix(f doctorFacts, connectFix string) string {
	...
	return "jevlin enroll -config <file>"
}
...
func doctorJoinedCheck(f doctorFacts) doctorCheck {
	...
	default:
		...
		c.Fix = "wait for the operator to open the next target, then: jevlin join -config <file>"
	}
	return c
}
...
func doctorPayoutCheck(f doctorFacts) doctorCheck {
	...
	case f.Standing == nil || (f.Standing.Active == nil && f.Standing.Pending == nil):
		...
		c.Fix = "jevlin payout set <twilight1...>   (or: jevlin wallet register)"
...
func doctorEarningCheck(f doctorFacts) doctorCheck {
	...
	if f.Activity.Eligible() {
		...
		c.Detail = fmt.Sprintf("epoch %d — %s verified; %d qualifies",
			f.Epoch, plural(f.Activity.VerifiedObservationCount, "observation"), auth.MinVerifiedObservations)
		c.Fix = "searches have to go through it: run one from an agent, or by hand: jevlin search -format model \"<query>\"  (then: jevlin flush)"
	}
...
func printDoctor(w io.Writer, checks []doctorCheck, f doctorFacts) {
	...
	fmt.Fprintln(w, "      jevlin earnings")
}
...
func probeIntakeWritable(ops intakeProbeOps, dir string) intakeProbeResult {
	...
	werr := ops.writeAtomic(dir, name, []byte("jevlin doctor probe\n"), 0o600)
...
func doctorIntakeCheck(f doctorFacts) doctorCheck {
	...
	if sandboxDenied(p.Err) {
		c.Fix = fmt.Sprintf("if searches run under Codex, re-run `jevlin agents install` so the sandbox allows %s", p.Dir)
	}
	return c
}
...
func doctorRecordingCheck(f doctorFacts) doctorCheck {
	...
	c.Detail = fmt.Sprintf("recent miner activity, but nothing is queued locally or verified at the AS; "+
		...
		"the agent's `jevlin search` command", f.IntakeDir)
	c.Fix = "jevlin agents status; re-run `jevlin agents install` if the agent is using another config or lacks sandbox access"
	return c
}
