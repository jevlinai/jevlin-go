...
func miningClients(ctx context.Context, args []string, cmd string) (*auth.OAuthClient, *auth.MiningClient, config.Mining, int) {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "jevlin: key store:", err)
		return nil, nil, config.Mining{}, 1
	}
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "jevlin: discovery:", err)
		return nil, nil, config.Mining{}, 1
	}
	oauthClient, err := auth.NewOAuthClient(ctx, disc, store)
	if err != nil {
		fmt.Fprintln(os.Stderr, "jevlin: authorization server:", err)
		return nil, nil, config.Mining{}, 1
	}
...
func resolveEpoch(ctx context.Context, mining *auth.MiningClient, m config.Mining) (uint64, bool) {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "jevlin: current target:", err)
	...
	if t == nil {
		fmt.Fprintln(os.Stderr, "jevlin: the AS reports no open target for this slot right now")
		return 0, false
	}
...
func cmdEnroll(args []string) int {
	...
	if *browser && *assertion {
		fmt.Fprintln(os.Stderr, "jevlin: -browser and -assertion are different enrollment doors; choose one")
		return 2
	}
	...
	case *assertion:
		...
		if err != nil && strings.TrimSpace(line) == "" {
			fmt.Fprintln(os.Stderr, "jevlin: no enrollment token on stdin")
			return 2
		}
		if _, err := oauthClient.RedeemEnrollmentAssertion(ctx, strings.TrimSpace(line)); err != nil {
			fmt.Fprintln(os.Stderr, "jevlin: enrollment failed:", err)
	...
	case *browser:
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "jevlin: could not start authorization:", err)
			return 1
		}
		defer pending.Close()
		...
		if _, err := pending.Wait(ctx); err != nil {
			fmt.Fprintln(os.Stderr, "jevlin: authorization failed:", err)
			return 1
		}
	default:
		da, err := oauthClient.StartDeviceAuthorization(ctx)
		if err != nil {
			fmt.Fprintln(os.Stderr, "jevlin: could not start the device flow:", err)
			return 1
		}
		...
		if _, err := oauthClient.WaitForDeviceApproval(ctx, da); err != nil {
			fmt.Fprintln(os.Stderr, "jevlin: approval failed:", err)
			return 1
		}
	}
	...
	fmt.Fprintln(os.Stdout, "next: jevlin join -config <file>")
	return 0
}
...
func cmdJoin(args []string) int {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "jevlin: epoch status:", err)
		return 1
	}
	...
	if !st.Joinable {
		fmt.Fprintln(os.Stderr, "jevlin: the AS reports this target is not joinable; nothing was sent")
		return 1
	}

	res, err := mining.JoinEpoch(ctx, epoch)
	if err != nil {
		fmt.Fprintln(os.Stderr, "jevlin: join:", err)
		return 1
	}
	...
	fmt.Fprintln(os.Stdout, "next: jevlin provider -config <file>   (reads the key from stdin)")
	return 0
}
...
func cmdProvider(args []string) int {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "jevlin: could not read the AS service document:", err)
		return 1
	}
	if !accepts {
		fmt.Fprintln(os.Stderr, "jevlin: this Slot does not accept OPENROUTER_V1 observations, "+
	...
	if *show {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "jevlin: provider status:", err)
			return 1
		}
		printBinding(b)
		return 0
	...
	line, err := bufio.NewReader(os.Stdin).ReadString('\n')
	if err != nil && strings.TrimSpace(line) == "" {
		fmt.Fprintln(os.Stderr, "jevlin: no key on stdin")
		return 2
	}
	key := strings.TrimSpace(line)
	if key == "" {
		fmt.Fprintln(os.Stderr, "jevlin: empty key")
		return 2
	}

	b, err := mining.RegisterProviderCredential(ctx, key)
	if err != nil {
		fmt.Fprintln(os.Stderr, "jevlin: register provider credential:", err)
		return 1
	}
	printBinding(b)
...
func renderStatusAS(f statusASFacts, stdout, stderr io.Writer) {
	...
	if !f.EpochKnown {
		if f.EpochErr != nil {
			fmt.Fprintln(stderr, "jevlin: current target:", f.EpochErr)
			...
			fmt.Fprintln(stderr, "jevlin: the AS reports no open target for this slot right now")
		}
	...
	if f.StatusErr != nil {
		...
		fmt.Fprintln(stdout, "\nnext: jevlin enroll -config <file>")
		return
	}
	st := f.Status
...
func cmdPayout(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "jevlin: payout needs a subcommand: set <address>, or show")
		return 2
	}
	...
	case "set":
		...
		if len(rest) == 0 || strings.HasPrefix(rest[0], "-") {
			fmt.Fprintln(os.Stderr, "jevlin: payout set needs an address: jevlin payout set <address> [-config file]")
			return 2
		}
		address, rest = rest[0], rest[1:]
	case "show":
	default:
		fmt.Fprintf(os.Stderr, "jevlin: unknown payout subcommand %q; want set or show\n", sub)
		return 2
	}
	...
	if sub == "show" {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "jevlin: payout status:", err)
			return 1
		}
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "jevlin: payout declaration failed:", err)
		return 1
	}
...
func printStanding(s *auth.PayoutStanding) {
	...
	case s.Active == nil && s.Pending == nil:
		...
		fmt.Fprintln(os.Stdout, "  propose one: jevlin payout set <twilight1...>")
		return
