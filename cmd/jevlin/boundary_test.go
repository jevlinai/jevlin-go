...
const module = "github.com/jevlinai/jevlin-go"
...
func TestSelfupdateImportsNoWrapperAndNoCredential(t *testing.T) {
	...
	for _, dep := range deps {
		switch dep {
		case module + "/cmd/jevlin", module + "/pkg/auth", module + "/pkg/platform":
...
const netdialImportPath = `"github.com/jevlinai/jevlin-go/internal/netdial"`
...
func TestConnectAndMiningNeverImportOSExec(t *testing.T) {
	...
	for _, name := range []string{"connect.go", "mining.go"} {
		path := filepath.Join(root, "cmd", "jevlin", name)
...
const unsafeAllowed = "cmd/jevlin/setup_env_windows.go"
