...
const (
	profileMarkerStart = "# >>> jevlin >>>"
	profileMarkerEnd   = "# <<< jevlin <<<"
	profileBlockNote   = "# Written by jevlin setup. Delete this block to undo."
)
...
// well-formed block.
var errProfileMalformed = errors.New("its jevlin markers are not one well-formed block")

// rewriteProfile returns the profile with exactly one jevlin block
...
// jevlin block, and the block's own lines. Zero markers is found=false
