...
func cmdEarnings(args []string, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if *limit < 0 {
		fmt.Fprintln(stderr, "jevlin: -limit cannot be negative; 0 lists every receipt")
		return exitUsage
	}
	...
	if target == "" || describeConfigSource(*cfgPath, getenv) != "" {
		...
		case target == "":
			...
			fmt.Fprintln(stderr, "jevlin: with no config there is no AS to ask where you are paid.")
			fmt.Fprintln(stderr, "  Name the address yourself: jevlin earnings -address <twilight1...>")
			return code
		}
	}
	...
	if err != nil {
		fmt.Fprintln(stderr, "jevlin: cannot read this address's transactions:", redact.Error(err))
...
func earningsAddressFromAS(ctx context.Context, as asClient, stdout, stderr io.Writer) (string, int) {
	if as == nil {
		fmt.Fprintln(stderr, "jevlin: no AS to ask where you are paid.")
		fmt.Fprintln(stderr, "  Name the address yourself: jevlin earnings -address <twilight1...>")
		return "", exitTransport
	}
	standing, err := as.PayoutStanding(ctx)
	if err != nil {
		fmt.Fprintln(stderr, "jevlin: could not ask the AS where you are paid:", redact.Error(err))
		fmt.Fprintln(stderr, "  It works offline if you name the address: jevlin earnings -address <twilight1...>")
		return "", exitTransport
	}
	...
	fmt.Fprintln(stdout, "\n  Register one:   jevlin payout set <twilight1...>")
	fmt.Fprintln(stdout, "                  jevlin wallet register   (if this machine holds the wallet)")
	return "", exitOK
}
...
func printEarningsUnlabeled(w io.Writer, r *earningsReport) {
	...
	fmt.Fprintln(w, "      jevlin earnings -escrow <twilight1...>")
}
