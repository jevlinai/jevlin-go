...
const setupUsage = `usage: jevlin setup [-home dir] [-yes] [-no-profile] [-no-agents] [-with id]... [-dry-run]
...
// global one.
var errNpmLaunch = errors.New("setup must run from a global npm install: npm install -g jevlin, then jevlin setup")
...
var errNpmDirect = errors.New("this is npm's copy of the binary, run directly; run the jevlin command npm installed, which tells setup how it was installed")
...
func setupMain(d setupDeps, args []string) int {
	...
	if fs.NArg() > 0 {
		fmt.Fprintf(d.stderr, "jevlin setup: unexpected argument %q\n%s", fs.Arg(0), renderSetupUsage())
		return exitUsage
	}
...
func (r *setupRun) run(homeFlag string, with []string) int {
	...
	if err != nil {
		fmt.Fprintln(d.stderr, "jevlin setup: cannot determine my own path:", err)
	...
	if err != nil {
		fmt.Fprintln(d.stderr, "jevlin setup: -with:", err)
		return exitUsage
	}
	...
	if r.dry {
		...
		if code := d.connect([]string{"-config", r.cfgPath}, d.stdin, d.stdout, d.stderr, getenv); code != exitOK {
			fmt.Fprintf(d.stderr, "\njevlin setup: connect failed (exit %d); run setup again when that is fixed — it picks up where this left off\n", code)
			return exitTransport
		}
	}
...
func (r *setupRun) admit() (release func(), code int) {
	...
	if err != nil {
		fmt.Fprintf(r.d.stderr, "jevlin setup: %v (%s); nothing was changed, run setup again shortly\n", err, gatePath)
		return nil, exitTransport
	}
	defer gate.release()
	if !lexists(r.home) {
		if err := os.MkdirAll(r.home, 0o700); err != nil {
			fmt.Fprintln(r.d.stderr, "jevlin setup:", err)
			return nil, exitTransport
		}
		r.changed = true
	}
	if err := restrictToOwner(r.home, true); err != nil {
		fmt.Fprintf(r.d.stderr, "jevlin setup: restrict %s to its owner: %v\n", r.home, err)
		return nil, exitTransport
	}
	lockPath := filepath.Join(r.home, setupLockFile)
	lock, held, err := tryLockFile(lockPath)
	if err != nil {
		fmt.Fprintln(r.d.stderr, "jevlin setup: lock:", err)
		return nil, exitTransport
	}
	if !held {
		fmt.Fprintf(r.d.stderr, "jevlin setup: another setup is already running for %s (%s is held); nothing was changed\n", r.home, lockPath)
		return nil, exitTransport
	}
...
func printAdoptionError(w io.Writer, err error) {
	...
	default:
		fmt.Fprintf(w, "\njevlin setup: adoption stopped: %v\nconnect was not run.\n", err)
	}
}
...
func printAdoptionFailure(w io.Writer, f *adoptionFailure) {
	fmt.Fprintf(w, "\njevlin setup: could not adopt the %s\n  from %s\n  into %s\n  because: %s\n",
...
func printIdentityConflict(w io.Writer, c *identityConflict) {
	fmt.Fprintf(w, "\njevlin setup: two installations each hold an identity (a registration and its key):\n"+
		"  %s  (holds %s)\n  %s\n"+
...
func (r *setupRun) walletAccess() int {
	...
	if err != nil {
		fmt.Fprintf(r.d.stderr, "\njevlin setup: could not make the wallet in %s readable only by you:\n%s\n"+
...
func (r *setupRun) config() int {
	...
	if err != nil {
		fmt.Fprintln(r.d.stderr, "jevlin setup: refusing the config:", err)
		return exitTransport
	}
...
func (r *setupRun) flushLock() int {
	...
	if err != nil {
		fmt.Fprintf(r.d.stderr, "jevlin setup: create the flush lock %s: %v\n", path, err)
		return exitTransport
	}
