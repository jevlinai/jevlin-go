...
func TestHermesHookCommandSurvivesYAMLAndTheArgvSplitter(t *testing.T) {
	...
	if _, ok := hermesHookCommand(hermesEntry(`C:\a"b\jevlin.exe`, ""), true); ok {
...
func TestHermesHookYAMLIsTheCommandWeMeant(t *testing.T) {
	body, ok := hermesHookYAML(hermesEntry("/home/u/O'Neil/jevlin", "/home/u/c.toml"))
	if !ok {
	...
	if argv[0] != "/home/u/O'Neil/jevlin" {
...
func TestHermesStatusRecognizesOurHookForAnyBinaryPath(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"backslash":  {`/home/u/odd\path/jevlin`, false},
		"hash":       {"/home/u/#1/jevlin", false},
	} {
		t.Run(name, func(t *testing.T) {
			m, ops := newFakeMachine("hermes")
