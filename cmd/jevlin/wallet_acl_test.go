...
func TestDoctorWalletAccessCheck(t *testing.T) {
	...
	cases := []struct {
		...
		{"another reader", walletAccessFacts{Checked: true, Dir: dir, Present: true, Readers: []walletReader{{Path: key, Principals: []string{`NT AUTHORITY\LogonSessionId_0_1`}}}},
			verdictNo, []string{key, `NT AUTHORITY\LogonSessionId_0_1`, recurringEntryDetail}, []string{"jevlin setup"}},
		{"a link", walletAccessFacts{Checked: true, Dir: dir, Present: true, Unsecurable: []string{filepath.Join(dir, "link")}},
			verdictNo, []string{filepath.Join(dir, "link")}, []string{"move " + filepath.Join(dir, "link"), "jevlin setup"}},
		...
		{"a reader outranks what could not be read", walletAccessFacts{Checked: true, Dir: dir, Present: true, Readers: []walletReader{{Path: key, Principals: []string{"S-1-5-5-0-1"}}}, Err: errors.New("x")},
			verdictNo, []string{key, recurringEntryDetail, "could not be checked"}, []string{"jevlin setup"}},
	}
	for _, c := range cases {
	...
	if lastJSON["name"] != "wallet access" || lastJSON["verdict"] != string(verdictNo) || lastJSON["fix"] != "jevlin setup" {
...
const recurringEntryDetail = "another program or a machine policy can add such an entry again, and jevlin setup re-applies the protection whenever it does"
