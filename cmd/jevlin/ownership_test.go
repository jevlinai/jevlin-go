...
func TestAnIntegrationIsOursOnlyWhenItNamesOurBinaryAndOurConfig(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"another binary, our config", posixQuoteArg("/opt/other/jevlin") + " hook -config " + posixQuoteArg(ours) + " lineage", false},
...
func TestTheCodexSandboxBlockIsAttributedByTheDirectoriesItsConfigNames(t *testing.T) {
	...
	entry := binEntry{command: filepath.Join(home, "bin", "jevlin"), cfg: cfg}
