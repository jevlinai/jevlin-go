...
func TestUpgradeRefusesEveryCopyThePackageManagerOwns(t *testing.T) {
	root := t.TempDir()
	native := filepath.Join(root, "home", "bin", "jevlin")
	...
	for name, tc := range map[string]struct {
		...
		"npm global":        {native, "npm:global", "npm install -g jevlin@latest"},
		...
		"npm exec cache":    {filepath.Join(root, "_npx", "1", "jevlin"), "", "package manager that installed it"},
		"node_modules":      {filepath.Join(root, "node_modules", "jevlin", "bin", "jevlin"), "", "package manager"},
		"npm layout":        {filepath.Join(layout, "bin", "jevlin"), "", "package manager"},
		"ambiguous layout":  {filepath.Join(partial, "bin", "jevlin"), "", "unclear"},
...
func TestVersionOutputIsAReleaseCompatibilityContract(t *testing.T) {
	...
	bin := filepath.Join(t.TempDir(), "jevlin")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	build := exec.Command("go", "build", "-trimpath", "-ldflags", "-X main.version=0.3.0", "-o", bin, "./cmd/jevlin") // #nosec G204 -- fixed arguments
	build.Dir = root
	...
	if err := cmd.Run(); err != nil || stdout.String() != "jevlin 0.3.0\n" || stderr.Len() != 0 {
		t.Errorf("version: err=%v stdout=%q stderr=%q; want exactly %q and nothing on stderr", err, stdout.String(), stderr.String(), "jevlin 0.3.0\n")
	}
}
