...
//
//	jevlin search [-tier fast] [-format json|model] <query words>
//
...
func searchMain(ops searchOps, args []string, stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if machine {
		...
		if query == "" {
			fmt.Fprintln(stderr, "jevlin search: a query is required: jevlin search [-tier fast] [-format model] <query words>, or --stdin")
			return exitUsage
		}
	...
	if *timeout <= 0 {
		...
		fmt.Fprintf(stderr, "jevlin search: -timeout must be positive, not %s\n", *timeout)
		return exitUsage
	}
...
func renderSearchForHuman(out searchOutcome, format string, keySrc keySource, stdout, stderr io.Writer) int {
	...
	switch out.Fault {
	case faultTimeout:
		fmt.Fprintln(stderr, "jevlin: the search did not finish within its deadline; retry, or raise -timeout")
		return exitTransport
	case faultCanceled:
		fmt.Fprintln(stderr, "jevlin: the search was canceled")
		return exitTransport
	case faultTransport:
		fmt.Fprintln(stderr, "jevlin: router:", out.Err)
		return exitTransport
	case faultOversized, faultInvalidResponse, faultMissingID:
		fmt.Fprintf(stderr, "jevlin: the router answered HTTP %d, but the answer is not a usable search response: %v\n",
...
func recordSearchForMining(ops searchOps, cfg *config.Config, out searchOutcome, cfgPath string, noFlush bool, stderr io.Writer) machineMining {
	...
	if decision.State == auth.MiningDegraded {
		...
		fmt.Fprintf(stderr, "jevlin: the search succeeded, but mining capture is unavailable because local mining state cannot be safely trusted%s\n",
			detail)
	}
	...
	if _, err := writeIntake(cfg.Miner.IntakeDir, rec); err != nil {
		...
		if intakeWriteBlocked(err) {
			...
			fmt.Fprintf(stderr, "jevlin: the search worked, but its mining observation could NOT be\n"+
				...
				"  For Codex, re-run `jevlin agents install`, which now configures it.\n",
				minerRoot(cfg.Miner))
		} else {
			fmt.Fprintln(stderr, "jevlin search: could not record the request for mining:", err)
		}
		if mstore != nil {
	...
	if !noFlush && ops.spawnFlush != nil {
		if err := ops.spawnFlush(cfgPath); err != nil {
			...
			fmt.Fprintln(stderr, "jevlin search: could not start mining flush:", err)
		}
	}
	return finish()
}
