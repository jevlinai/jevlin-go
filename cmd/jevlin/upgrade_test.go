...
func (lr *localRelease) archive(version string) ([]byte, string) {
	...
	_ = tw.WriteHeader(&tar.Header{Name: "jevlin", Mode: 0o755, Size: int64(len(body)), Typeflag: tar.TypeReg})
	_, _ = tw.Write([]byte(body))
	_ = tw.Close()
	_ = gz.Close()
	return out.Bytes(), fmt.Sprintf("jevlin_%s_linux_amd64.tar.gz", version)
}
...
func (c *contentRunner) Run(ctx context.Context, path string, _, _ []string) ([]byte, []byte, error) {
	...
	return []byte("jevlin " + strings.TrimSpace(string(b)) + "\n"), nil, nil
}
...
func TestUpgradeCommandRefusesADowngrade(t *testing.T) {
	...
	if code != upgradeRefused.exit() || !strings.HasPrefix(errOut, "jevlin upgrade: refused: ") || f.content(f.exe) != "0.3.0" {
...
func TestUpgradeCommandSendsNpmCopiesToNpmBeforeAnythingElse(t *testing.T) {
	f := newUpgradeFixture(t, newLocalRelease(t, "0.3.1", map[string]string{"0.3.1": "0.3.1\n"}))
	f.env["JEVLIN_LAUNCH"] = "npm:global"
	code, out, errOut := f.run()
	if code != exitUsage || !strings.HasPrefix(errOut, "jevlin upgrade: ownership: ") || !strings.Contains(errOut, "npm install -g jevlin@latest") {
...
func TestUpgradeCommandRefusesADevBuildBeforeAnything(t *testing.T) {
	...
	code, _, errOut := f.run()
	if code != exitUsage || !strings.HasPrefix(errOut, "jevlin upgrade: ownership: ") {
...
func TestUpgradeCommandRefusesUnderAHeldLifecycleGateOfItsCustomHome(t *testing.T) {
	...
	code, _, errOut := f.run()
	if code != exitTransport || !strings.HasPrefix(errOut, "jevlin upgrade: lifecycle_busy: ") {
...
func TestUpgradeCommandFlagCombinations(t *testing.T) {
	...
	for name, args := range map[string][]string{
		...
		if code, _, errOut := f.run(args...); code != exitUsage || !strings.Contains(errOut, "usage: jevlin upgrade") {
...
func TestUpgradeCommandPrintsSuccessOnlyAfterTheCanonicalPathValidates(t *testing.T) {
	...
	if code != exitTransport || !strings.HasPrefix(errOut, "jevlin upgrade: retry: ") {
...
func TestTopLevelHelpNamesTheLifecycleCommands(t *testing.T) {
	...
	for _, want := range []string{"-purge-state", "-yes never answers it", "-rollback", "npm install -g jevlin@latest"} {
