...
func TestResolveAPIKeyRefusesAFileOthersCanRead(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "jevlin login") {
...
func TestResolveAPIKeyRefusesASymlinkAndACorruptFile(t *testing.T) {
	...
	if _, _, err := resolveAPIKey(noEnv, m); err == nil || !strings.Contains(err.Error(), "jevlin login") {
...
func TestLoginShowAndForget(t *testing.T) {
	...
	if code != exitOK || !strings.Contains(out, "no key") || !strings.Contains(out, "jevlin login") {
