...
// pre_tool_call entry running `jevlin hook hermes pre_tool_call`.
//
...
func planHermesHookFor(ops agentOps, label, path string, entry binEntry, windows bool, p *agentPlan) bool {
	...
	if own.found {
		...
		p.notes = append(p.notes, fmt.Sprintf(
			"%s: already set up — the pre_tool_call hook is in %s at %s, under a hooks: block jevlin did not write; left as it is",
	...
	if reason := hermesConfigRefusal(stripped); reason != "" {
		if own.mention > 0 {
			...
			p.refused = append(p.refused, fmt.Sprintf(
				"%s: %s %s, and line %d of it already names this installation's pre_tool_call hook command in a place or a form jevlin cannot read reliably; check whether it is already set up before adding this entry by hand:\n%s",
...
// quote-escape-quote run, so a path like /Users/O'Neil/bin/jevlin
...
func hermesConfigScan(b []byte) (reason string, hooks int) {
	if bytes.Contains(b, []byte(agentsMarkerBegin)) || bytes.Contains(b, []byte(agentsMarkerEnd)) {
		return "carries a partial jevlin block we cannot read back", 0
	}
...
func hermesBlockLeftNote(label, path, why string) string {
	return fmt.Sprintf("%s: left the jevlin pre_tool_call hook block in %s as it is, because %s", label, path, why)
}
...
func hermesBlockLeftBecause(lines []hermesLine, m hermesMarkers, ref installationRef) string {
	...
	for i, l := range block {
		if l.indent < 0 && (i != 0 || l.text != hermesNoEOLNote) {
			return named(i) + " is between the markers and is not a line jevlin writes, and would go with the block"
		}
	}
	cmd, bad := hermesReadMarkedBlock(block)
	switch {
	case bad >= len(block):
		return "it no longer holds the whole entry jevlin writes between its markers"
	case bad >= 0:
		return named(bad) + " is between the markers and is not a line jevlin writes, and would go with the block"
	}
...
func hermesWhoseHook(cmd string) string {
	...
	return "the command between its markers is not one jevlin writes (" + cmd + ")"
}
...
func planHermesUnhook(ops agentOps, label, path string, entry binEntry, p *agentPlan) (removed, noted bool) {
	...
	case own.removable():
		...
		planWrite(ops, label, path, removeHermesOwnEntry(existing, own), mode,
			"remove lineage hook from a hooks: block jevlin did not write; every other line is kept as it is", p)
		return true, false
	case own.found:
		p.notes = append(p.notes, fmt.Sprintf(
	...
	case own.mention > 0:
		p.notes = append(p.notes, fmt.Sprintf(
			"%s: line %d of %s names this installation's pre_tool_call hook command, in a place or a form jevlin cannot read reliably, so nothing there was changed; if Hermes still runs it, remove it by hand",
...
func hermesFindStructured(b []byte, ref installationRef) hermesOwnEntry {
	...
	if !ok {
		return leave("does not have the matcher line jevlin writes under it, so it has been edited")
	}
	if next := hermesNextContent(lines, end); next >= 0 && lines[next].indent > 4 {
		return leave("has a further key inside it that jevlin did not write")
	}
	...
	if !hermesRunIsOurs(lines, entry, at, ref) {
		...
		return leave("is not written exactly as jevlin writes it, nor as Hermes rewrites it when it saves config.yaml")
	}
	return entry
}
