...
func cmdWallet(args []string, stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "jevlin: wallet needs a subcommand: init, address, register, balance or send")
		return exitUsage
	}
	...
	default:
		fmt.Fprintf(stderr, "jevlin: unknown wallet subcommand %q; want init, address, register, balance or send\n", sub)
		return exitUsage
	}
}
...
func walletInit(args []string, stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if _, err := os.Lstat(filepath.Join(resolved, walletKeyFile)); err == nil {
		if sc, _, serr := loadSidecar(resolved, getenv); serr == nil && sc != nil {
			fmt.Fprintf(stderr, "jevlin: a wallet already exists in %s; refusing to overwrite it.\n"+
	...
	if !*printAnyway && !isTerminal(stdout) {
		fmt.Fprintln(stderr, "jevlin: stdout is not a terminal; the mnemonic would be printed into a capture.\n"+
	...
	fmt.Fprintln(stdout, "    jevlin wallet register -config <file>")
	return 0
}
...
func walletRegister(args []string, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if err != nil {
		fmt.Fprintln(stderr, "jevlin: payout declaration failed:", err)
		return exitTransport
	}
...
func walletPassphrase(stdin io.Reader, br *bufio.Reader, stderr io.Writer, getenv func(string) string, create bool) (string, int) {
	...
	fmt.Fprintln(stderr)
	if err != nil && first == "" {
		fmt.Fprintln(stderr, "jevlin: no passphrase provided (set "+walletPassphraseEnv+" for non-interactive use)")
		return "", exitUsage
	}
	if first == "" {
		fmt.Fprintln(stderr, "jevlin: an empty passphrase would store the key effectively unencrypted; refusing")
		return "", exitUsage
	}
	if !create {
		return first, 0
	...
	fmt.Fprintln(stderr)
	if second != first {
		fmt.Fprintln(stderr, "jevlin: passphrases do not match")
		return "", exitUsage
	}
	return first, 0
}
...
func walletBalance(args []string, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if _, _, err := auth.DecodeBech32Address(target); err != nil {
		fmt.Fprintf(stderr, "jevlin: %q is not a valid address: %v\n", target, err)
		return exitUsage
	}
	...
	if err != nil {
		fmt.Fprintln(stderr, "jevlin: balance:", err)
		return exitTransport
	}
...
func walletSend(args []string, stdin io.Reader, stdout, stderr io.Writer, getenv func(string) string) int {
	...
	if *to == "" || *amount == "" {
		fmt.Fprintln(stderr, "jevlin: wallet send needs -to <address> and -amount <integer>")
		return exitUsage
	}
	hrp, _, err := auth.DecodeBech32Address(*to)
	if err != nil {
		fmt.Fprintf(stderr, "jevlin: -to %q is not a valid address: %v\n", *to, err)
		return exitUsage
	}
	...
	if hrp != auth.TwilightHRP {
		fmt.Fprintf(stderr, "jevlin: -to is a %q address, not %q; refusing to send across chains\n", hrp, auth.TwilightHRP)
		return exitUsage
	}
	if !isPositiveInteger(*amount) {
		fmt.Fprintf(stderr, "jevlin: -amount %q must be a positive integer of %s (the base denomination, not a decimal)\n", *amount, *denom)
		return exitUsage
	}
	if !isPositiveInteger(*fee) {
		fmt.Fprintf(stderr, "jevlin: -fee %q must be a positive integer\n", *fee)
		return exitUsage
	}
	if sc.Address == *to {
		fmt.Fprintln(stderr, "jevlin: -to is this wallet's own address; nothing to do")
		return exitUsage
	}
	...
	if expectedChainID == "" {
		fmt.Fprintln(stderr, "jevlin: no chain id to sign for — pass -chain-id, or -config naming a [mining] chain_id")
		return exitUsage
	}
	...
	if err != nil {
		fmt.Fprintln(stderr, "jevlin: cannot reach the node:", err)
		return exitTransport
	}
	if reportedChainID != expectedChainID {
		fmt.Fprintf(stderr, "jevlin: node %s reports chain %q, expected %q; refusing to sign\n", nodeURL, reportedChainID, expectedChainID)
		return exitUsage
	}
	...
	if err != nil {
		if errors.Is(err, errNoAccount) {
			fmt.Fprintf(stderr, "jevlin: %s has never received funds, so there is nothing to send\n", sc.Address)
			return exitTransport
		}
		fmt.Fprintln(stderr, "jevlin: account:", err)
		return exitTransport
	}
	...
	if !*yes {
		...
		if err != nil {
			...
			fmt.Fprintf(stderr, "jevlin wallet send: %s; nothing was signed or sent\n", promptAbortedReason)
			return exitUsage
		}
	...
	if err := readWalletFile(resolved, walletKeyFile, &kf); err != nil {
		fmt.Fprintln(stderr, "jevlin: keyfile:", err)
		return exitTransport
	}
	key, err := auth.OpenWalletKey(&kf, passphrase)
	if err != nil {
	...
	if err != nil {
		fmt.Fprintln(stderr, "jevlin: build transaction:", err)
		return exitTransport
	}
	...
	if err := writePendingTx(resolved, pendingTx{
		...
		fmt.Fprintln(stderr, "jevlin: could not journal this transaction before sending it:", err)
		return exitTransport
	}
	...
	if err != nil {
		...
		fmt.Fprintf(stderr, "jevlin: broadcast: %v\noutcome unknown: %s\n", err, hash)
		return exitOutcomeUnknown
	}
	if res.Code != 0 {
		...
		if rerr := removePendingTx(resolved); rerr != nil {
			fmt.Fprintln(stderr, "jevlin:", rerr)
		}
		fmt.Fprintf(stderr, "jevlin: the chain rejected this transaction (code %d): %s\n", res.Code, res.Log)
	...
	if err != nil {
		fmt.Fprintf(stderr, "jevlin: submitted, but it has not appeared in a block yet: %v\n", err)
	...
	func() {
		...
		if lockErr != nil {
			fmt.Fprintln(stderr, "jevlin: could not lock the confirmed pending transaction for removal:", lockErr)
			return
		}
		...
		if !removed && currentHash != "" {
			fmt.Fprintf(stderr, "jevlin: confirmed transaction %s no longer owns the pending journal; leaving replacement %s intact\n", hash, currentHash)
		}
	}()
	if tx.TxResult.Code != 0 {
		fmt.Fprintf(stderr, "jevlin: included at height %s but FAILED (code %d): %s\n",
...
func validateNodeURL(raw string, insecure bool, stderr io.Writer) error {
	...
	if insecure {
		fmt.Fprintf(stderr, "jevlin: -insecure-node: %s is not https and not loopback; sending the signed transaction and reading balances over it anyway\n", raw)
		return nil
	}
