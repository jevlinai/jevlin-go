...
// recognizes is what install wrote on that OS by construction. Two spellings
// are not a shell's: Go's %q and the bare path, both of which Claude Code's
// allow rules are written in (claudeAllowRules), the bare path also being
...
// PowerShell), a double-quoted word (cmd, %q and JSON), and a bare word — which is
...
// strips the quotes and takes the rest literally, while %q and JSON escape,
// and a Windows path spelled either way names the same file.
...
// installation's.
//
// The comparison is samePath, not bytes: %q hands Windows a path with doubled
// separators naming the same file, and Windows paths differ in case without
// differing.
...
// Windows paths differ in case without differing, and a separator may be
// spelled more than one way — the same reason namesOurConfig compares with
// samePath rather than bytes.
